    ...
    if (post_type === 'professors') {
      return `
			<ul class="link-list min-list">
				${results.length > 0 ? results.map(item => `<li class="professor-card__list-item">
      ...
      return `
			<ul class="link-list min-list">
				${results.length > 0 ? results.map(item => `<li>
						<a href="${item.permalink}">${item.title}</a> ${item.hasOwnProperty("postType") ? item.postType === 'post' ? 'by ' + item.authorName : "" : ""}
					</li>`).join('') : `<p>No ${format_posttype} match that search. <a href="${universityData.root_url}/${post_type}">View all ${format_posttype}</a></p>`}
			</ul>
			`;
    }
